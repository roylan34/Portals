...
if(Utils::getValue('doc_from') && Utils::getValue('doc_to'))	{ 
	$search ="AND doc_date BETWEEN '".$conn->escapeString(Utils::getValue('doc_from'))."' AND '".$conn->escapeString(Utils::getValue('doc_to'))."'"; 
			...
			if(!Utils::isEmpty($sap_code)){
				$conn->selectQuery('FORMAT(SUM(ROUND(vat,2)),2) AS total_vat, FORMAT(SUM(ROUND(gross,2)),2) AS total_gross, FORMAT(SUM(ROUND(net,2)),2) AS total_net ','sap_db.tbl_sales_history_auto_import WHERE id > 0 AND sap_code="'.$sap_code.'"');
			...
			$conn->selectQuery('*','sap_db.tbl_sales_history_auto_import WHERE id > 0 AND sap_code="'.$sap_code.'"');
			...
			$conn->selectQuery('*','sap_db.tbl_sales_history_auto_import WHERE id > 0 AND sap_code="'.$sap_code.'" '.$search.'' );
