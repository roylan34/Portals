...
WHERE pm.mif_id ='.$requestData['mif_id'].' AND ps.status = "close"');
...
$conn->selectQuery('pm.*',
	...
	LEFT JOIN tbl_company com ON pm.company_id = com.id
	LEFT JOIN tbl_accounts ac ON ps.technician = ac.id
	WHERE pm.mif_id = '.$requestData['mif_id'].' '.$search.' AND ps.status = "close"');
...
$conn->selectQuery('com.company_name, ps.schedule_date, CONCAT(ac.firstname," ", ac.lastname) AS technician, pm.remarks, pm.page_count, pm.time_in, pm.time_out',
	'tbl_pm_machines pm
	LEFT JOIN tbl_pm_schedule ps ON pm.pm_number = ps.pm_number
	LEFT JOIN tbl_company com ON pm.company_id = com.id
	LEFT JOIN tbl_accounts ac ON ps.technician = ac.id
	WHERE pm.mif_id ="'.$requestData['mif_id'].'" AND ps.status = "close" '.$search.'  ORDER BY pm.id DESC LIMIT '.$requestData['start'].' ,'.$requestData['length'].'');
