...
LEFT JOIN tbl_model mo ON im.model = mo.id
LEFT JOIN tbl_invnt_status s ON im.id_status = s.id
WHERE NOT EXISTS(SELECT serialnumber FROM sap_db.tbl_invnt_machines_auto_import ina WHERE im.serialnumber = ina.serialnumber) 
AND im.is_delete = 0 AND (im.id_status = 0 OR s.status_type = "IN")
) X GROUP BY branch, model ORDER BY id_branch_sort');
