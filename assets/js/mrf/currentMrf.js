	...
	* @param - {array}  paramObj.inputIdName
                    ...
                    if (view_data.res_row_data_s1.length > 0 && user_type == 5 && app_action.action_mrf == "wr") { // 5 = Engineering
                        $.each(view_data.res_row_data_s1, function (i, val) {
                            ...
                            edit_sn += '<td>' + serial.length + '</td>' +
                                '<td>' + val.brand_name + '</td>' +
                                '<td>' + val.model_name + '</td>' +
                                '<td data-row="bnrf"><div style="margin-top:12px;">' + autoDrpDownMrf.bnrf(val.s1_bn_rf) + '</div></td>';
    ...
    edit_engr: function (id_mrf, elemBtn) {
        ...
        $("#edit-table-sn tbody tr").each(function (i, $val) {
            var snArr = {};
            var bnrfArr = {};
            var id = $(this).attr('data-id-edit-sn');
            ...
            var bnrf = $(this).find("td[data-row^='bnrf'] select[name='s1_bn_rf'] option:selected").val();
            snArr["s1_serialnum"] = snStr;
            snArr["s1_bn_rf"] = bnrf;
            row_data.update[id] = snArr;

        });

        if (Object.keys(row_data.update).length > 0) {
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                data: { action: 'edit_approve_data', action_approver: 'engr', id_mrf: id_mrf, row_data_sn: row_data },
                url: assets + 'php/mrf/mrf.php',
                beforeSend: function () {
                    $btn.button('loading');
                },
