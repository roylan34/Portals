...
var dtCurrentMrf = {
    ...
    modalShow: function(mrf_flag){
    	...
    	if(mrf_flag == 'requestor'){
		    $("#displayFormMrfEntry").load(pages+'mrf/current/form.html',function(){
		    		...
		    		var tableComponent1 =  {elemTable: 'tbl_frm_component_1', elemBtnAdd : '#btn_add_row_s1', deleleteSubRow: true,
		    									 customCell: [ 
		    		...
		    		var tableComponent1Toner =  {elemTable: 'tbl_frm_s1_toner', elemBtnAdd : '#btn_add_row_s1_toner',
		    									    customCell: [ 
		    		...
		    		var tableComponent2 =  {elemTable: 'tbl_frm_component_2', elemBtnAdd : '#btn_add_row_s2',
								 customCell: [ 
								  				{cellNo: 0,
									  				elem: function(){ //custom elem brand
									  					 if(autoDrpDown.cacheOptBrands.length != 0){
	    ...
	    if(mrf_flag == 'approver'){
		    $("#displayViewStepDetails").load(pages+'mrf/current/view-request-form.html',function(){
					...
					$(this).find('#modalFormCurrentViewMrf').on('hidden.bs.modal', function() { //Reset form when modal hidden
					      ...
					      $(this).find(".pre-list-attachment").html('');
					});
					//Hide Toner modal
    ...
    add: function(callBackNavigateForm){ // IN-PROGRESS, attachment
    	 ...
    	 fileUpload.upload(files,{action:"add_files"}, function(){ //File attachment with callback inserting data.
    	 		...
    	 		if(status  == 'true'){
			    	...
			    	var data = {action:'add', s1_row_data: s1_row_data.insert, s1_row_toner_data: s1_row_toner_data, s2_row_data: removeEmptyObjectInArray(s2_row_data.insert,"s2_id_brand"), s2_radio_id:s2_radio_id, form_no: form_no,
    ...
    update: function(id,callBackAction){ // IN PROGRESS, attachment
	    	...
	    	fileUpload.upload(files,{action:"update_files", file_name:file_name}, function(){ //File attachment with callback updating data.
    	 		...
    	 		if(status  == 'true'){
				 	...
				 	var data = { action: 'update', id_mrf:id_mrf, s1_row_data: s1_row_data, s1_row_del_data: s1_row_del_data, 
    	    ...
    	    if (id != '' && id != undefined){
			 	...
			 	$.ajax({
	                ...
	                success: function(data, xhr, status){
	                	...
	                	if(user_type > 1){
	                		$(".attachment-upload-container, .remove-attachment").remove();
	                	}
	                	else{
    ...
    attachTableTonerData: function(id){ //HAVE BUG for fixing.
    	var ids = id;
    	    // ids = parseInt(ids);
    	// var row_index = null;
  		var s1_row_toner_data = getTableRowValue({elemTable: "#tbl_frm_s1_toner", trAttr: 'mrf-s1-toner', selectorInput: "input, select", inputElem: [{name: "s1_toner_qty", getValueByChosen: false},
  		...
  		var s1_row_toner_text = getTableRowText({elemTable: "#tbl_frm_s1_toner", trAttr: 'mrf-s1-toner', selectorInput: "input, select", inputElem: [{name: "s1_toner_qty", getValueByChosen: false},
    							{name: "s1_toner_monocolor", getValueByChosen: false}, {name:"s1_toner_model", getValueByChosen: false},{name:"s1_toner_price", getValueByChosen: false},{name:"s1_toner_yield", getValueByChosen: false},
    							{name:"s1_toner_total", getValueByChosen: false}, {name:"s1_toner_type", getValueByChosen: false}, {name:"s1_toner_rate", getValueByChosen: false}] });
