        ...
        dashboard: function (fullname, user_role, appmodule) { //Load dashboard template.
            ...
            $(".page-home").load(pages + 'index.html', function () {
                ...
                $(".view #include-left-sidebar").load(baseApp + 'pages/dashboard/left-sidebar.html', function () {
                    ...
                    //     views.find("ul.sidebar-menu li#sidebar-reports").remove();
                    // }

                    //Reports
                    var _module = jwt.get('app_module');
                    if (_module.app_reports == 0) {
                        views.find("ul.sidebar-menu li#sidebar-reports").remove();
                    }

                    var _action = jwt.get('app_module_action') || null;
                    if (_action != null) {

                        var parse_action = JSON.parse(_action.action_reports);

                        if (ObjhasProperty(parse_action, 'chkReportStocks') && parse_action.action.chkReportStocks != 'r') {
                            views.find("ul.sidebar-menu li#reports-sap-ho").remove();
                        }
                        if (ObjhasProperty(parse_action, 'chkReportDelivery') && parse_action.action.chkReportDelivery != 'r') {
                            views.find("ul.sidebar-menu li#reports-sap-deliveries").remove();
                        }
                        if (ObjhasProperty(parse_action, 'chkReportReceipts') && parse_action.action.chkReportReceipts != 'r') {
                            views.find("ul.sidebar-menu li#reports-sap-receipts").remove();
                        }
                        if (ObjhasProperty(parse_action, 'chkReportRefurbished') && parse_action.action.chkReportRefurbished != 'r') {
                            views.find("ul.sidebar-menu li#reports-in-out-stock").remove();
                        }
                        if (ObjhasProperty(parse_action, 'chkReportSummary') && parse_action.action.chkReportSummary != 'r') {
                    ...
                    if (user_role == "User") {
                        ...
                        if (app_module.app_invnt == 0) {
                            views.find("ul.sidebar-menu li#home-inventory, ul.sidebar-menu li#current-inventory, ul.sidebar-menu li#archive-inventory").remove(); //Remove sub-menu of Inventory
                        }
                        ...
                        if (app_module.app_invnt == 0) {
                            views.find("ul.sidebar-menu li#home-inventory, ul.sidebar-menu li#current-inventory, ul.sidebar-menu li#archive-inventory, ul.sidebar-menu li#settings-inventory").remove(); //Remove sub-menu of Inventory
                        }
                        if (app_module.app_mrf == 0) {
